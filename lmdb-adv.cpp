...
{
    // A transaction that uses this function must finish (either commit or abort) before any other transaction in the
...
MDBEnv::MDBEnv(const char *fname, int flags, int mode)
{
    ...
    /*
Various other options may also need to be set before opening the handle, e.g. mdb_env_set_mapsize(),
...
MDBDbi MDBEnv::openDB(const string_view dbname, int flags)
{
    ...
    /*
      ...
      uses this function must finish (either commit or abort) before any other transaction in the process may use this
      function.
    */
...
MDBRWTransactionImpl::MDBRWTransactionImpl(MDBEnv *parent, MDB_txn *txn): MDBROTransactionImpl(parent, txn) {}
...
MDB_txn *MDBRWTransactionImpl::openRWTransaction(MDBEnv *env, MDB_txn *parent, int flags)
{
    ...
    { // it might happen twice, who knows
...
int MDBRWTransactionImpl::commit()
{
    closeRORWCursors();
...
MDBROTransactionImpl::MDBROTransactionImpl(MDBEnv *parent, MDB_txn *txn): d_parent(parent), d_cursors(), d_txn(txn) {}
...
void MDBROTransactionImpl::abort()
{
    ...
    // if d_txn is non-nullptr here, either the transaction object was invalidated earlier (e.g. by moving from it), or
    // it is an RW transaction which has already cleaned up the d_txn pointer (with an abort).
    if (d_txn)
    {
        d_parent->decROTX();
...
int MDBROTransactionImpl::commit()
{
    int rc = MDB_BAD_TXN;
    ...
    if (d_txn)
    {
        ...
        rc = mdb_txn_commit(d_txn); // this appears to work better than abort for r/o database opening
...
int MDBRWTransactionImpl::clear(MDB_dbi dbi)
{
    return mdb_drop(d_txn, dbi, 0);
}
