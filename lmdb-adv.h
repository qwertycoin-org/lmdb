...
/*
The error strategy. Anything that "should never happen" turns into an exception. But things like 'duplicate entry' or
'no such key' are for you to deal with.
 */
...
class MDBEnv
{
  public:
    ...
    ~MDBEnv()
    {
        //    Only a single thread may call this function. All transactions, databases, and cursors must already be
...
class MDBROTransactionImpl
{
  ...
  public:
    ...
    int get(MDB_dbi dbi, const MDBInVal &key, MDBOutVal &val)
    {
        ...
        int rc = mdb_get(d_txn, dbi, const_cast<MDB_val *>(&key.d_mdbval), const_cast<MDB_val *>(&val.d_mdbval));
...
/*
   A cursor in a read-only transaction must be closed explicitly, before or after its transaction ends. It can be reused
...
template<class Transaction, class T> class MDBGenCursor
{
  ...
  public:
    MDBGenCursor(): d_registry(nullptr), d_cursor(nullptr) {}

    MDBGenCursor(std::vector<T *> &registry, MDB_cursor *cursor): d_registry(&registry), d_cursor(cursor)
    {
  ...
  public:
    ...
    MDBGenCursor(MDBGenCursor &&src) noexcept: d_registry(src.d_registry), d_cursor(src.d_cursor)
    {
        move_from(&src);
...
class MDBRWTransactionImpl : public MDBROTransactionImpl
{
  ...
  public:
    ...
    int clear(MDB_dbi dbi);

    int put(MDB_dbi dbi, const MDBInVal &key, const MDBInVal &val, int flags = 0)
    {
        if (!d_txn)
            throw std::runtime_error("Attempt to use a closed RW transaction for put");

        return mdb_put(d_txn, dbi, const_cast<MDB_val *>(&key.d_mdbval), const_cast<MDB_val *>(&val.d_mdbval), flags);
    }


    int del(MDBDbi &dbi, const MDBInVal &key, const MDBInVal &val)
    {
        return mdb_del(d_txn, dbi, (MDB_val *)&key.d_mdbval, (MDB_val *)&val.d_mdbval);
    }

    int del(MDBDbi &dbi, const MDBInVal &key)
    {
        return mdb_del(d_txn, dbi, (MDB_val *)&key.d_mdbval, 0);
    }
    ...
    int get(MDBDbi &dbi, const MDBInVal &key, MDBOutVal &val)
    {
        ...
        return mdb_get(d_txn, dbi, const_cast<MDB_val *>(&key.d_mdbval), const_cast<MDB_val *>(&val.d_mdbval));
    }
...
/* "A cursor in a write-transaction can be closed before its transaction ends, and will otherwise be closed when its
   transaction ends" This is a problem for us since it may means we are closing the cursor twice, which is bad
*/
class MDBRWCursor : public MDBGenCursor<MDBRWTransactionImpl, MDBRWCursor>
{
  public:
    ...
    int put(const MDBOutVal &key, const MDBInVal &data)
    {
        return mdb_cursor_put(
            *this, const_cast<MDB_val *>(&key.d_mdbval), const_cast<MDB_val *>(&data.d_mdbval), MDB_CURRENT);
    }
    ...
    int put(const MDBOutVal &key, const MDBOutVal &data, int flags = 0)
    {
        // XXX check errors
        return mdb_cursor_put(
            *this, const_cast<MDB_val *>(&key.d_mdbval), const_cast<MDB_val *>(&data.d_mdbval), flags);
    }
